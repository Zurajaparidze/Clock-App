...
function App() {
  ...
  return ( 
    <div className={`App ${currentTime.getHours() >= 23 || currentTime.getHours() < 11 ? "night" : "day"}`}>
      <div className="shadow">
       {showDiv ?<div className="row">
       <p className="text-p">
      ...
      <div className="row">
        <p className='morning-p'><img src={currentTime.getHours() >= 23 || currentTime.getHours() < 11 ? moon: sun} className="sun" />{currentTime.getHours() >= 23 || currentTime.getHours() < 11 ? "GOOD EVENING, IT’S CURRENTLY ": "GOOD MORNING, IT’S CURRENTLY"}</p>
      <p className="clock">{currentTime.toLocaleTimeString()}</p>
      <div className='footer-div'>
      <p className='location-p'>IN {typeof data === 'object' ?data?.city.toString().toUpperCase(): ''},{typeof data === 'object' ?data?.country_name.toString().toUpperCase(): ''}</p>
      <button className='button-more' onClick={()=>setShowDiv(!showDiv)}>{showDiv ? "MORE":"LESS"}<div className="circle"><img className='arrow-icon'src={showDiv ?arrowup :arrowdown} alt="datunia" /></div></button>
      </div>
      </div>
      {showDiv ? null : <div className="row3">
